//! M1 关键承诺验证：现有 `jarvis.db` 打开数据完好、仅新增 M1 观测表。
//!
//! 使用 SQLite backup API 将真实库复制到临时目录后打开（只读源库，
//! 不修改用户数据）。开发机存在真实 DB 时全量断言；CI/其他环境
//! 无此文件则自动跳过。
//!
//! M1 落地后契约更新：打开老库会幂等新增 3 张 LLM 指标表
//! （llm_calls / llm_tool_calls / llm_metrics_daily），除此之外
//! 零数据改动——这正是"观测层不碰用户数据"的承诺边界。
...
fn real_db_copy_opens_without_data_loss() {
    ...
    // 4. 表结构变化仅限 LLM 观测表（M1 的 3 张 + M3 的 2 张；27 业务表 + FTS 内部 4 + sqlite_sequence = 34）
    let tables: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table'",
            [],
        ...
        .unwrap();
    let new_tables = NEW_M1_TABLES.len() + NEW_M3_TABLES.len();
    assert_eq!(
        tables,
        src_tables + new_tables as i64,
        "表数量变化：打开老库应恰好新增 LLM 观测表（M1 3 张 + M3 2 张）"
    );
    assert_eq!(tables, 34);
    ...
    let hit: i64 = conn
        ...
        .unwrap()
        .query_row(["\"strategy_project\""], |r| r.get(0))
        .unwrap();
